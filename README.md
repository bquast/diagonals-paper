...
matrices from arrays is provided. The `matricise()` function takes
three- or four-dimensional array and outputs a matrix. Resizing or
transposing an array to a matrix is traditionally done using `dim()<-`
and `aperm` respectively, however ordering in of the data using these
methods does not create the fat matrix design that we are looking for.
...
`array`s using the `matricise()` function.
